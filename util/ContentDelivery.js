...
exports.Upload = (file) => {
    ...
    return new Promise((resolve,reject)=>{
        ...
        var genFileName = 'upload_' + generateFileRef()
        ...
        fs.writeFile('./storage/' + genFileName, file.data, (err)=>{
            if(err) reject(err)
            resolve(genFileName)
        })
    })
...
function generateFileRef() {
    ...
    let mins = date.getMinutes()>=9 ? date.getMinutes().toString() : '0' + date.getMinutes().toString()
